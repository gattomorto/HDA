...
class ResNet(SparseModel):
    ...
    def __init__(self, sparsity, num_classes=8, recompute_grad=False, name=None):
        super().__init__(name=name)
        self.num_classes = num_classes
        self.recompute = recompute_grad
        ...
        self.stage2 = ResNet.ResNetStack(64, 64, 3, stride1=1, sparsity=sparsity, recompute=recompute_grad, name="conv2")
        self.stage3 = ResNet.ResNetStack(256, 128, 4, stride1=2, sparsity=sparsity, recompute=recompute_grad, name="conv3")
        self.stage4 = ResNet.ResNetStack(512, 256, 6, stride1=2, sparsity=sparsity, recompute=recompute_grad, name="conv4")
        self.stage5 = ResNet.ResNetStack(1024, 512, 3, stride1=2, sparsity=sparsity, recompute=recompute_grad, name="conv5")
...
def test2(model, X, y, patch_size=32, stride=16, batch_size=32):
    ...
    has_violet = np.zeros(N, dtype=bool)
...
def main():
    (X_train, y_train), (X_test, y_test), (X_val, y_val) = load_bloodmnist_subset(patching=False)
    ...
    model = ResNet(sparsity = 0.0, recompute_grad=True)
